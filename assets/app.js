...
fileInput.on('change', function (e) {
    ...
    for(var j=0; j<files.length; j++){
        if(files[j].type.match(/audio\/(mp3|mpeg|m4a)/)){

            getID3Data(files[j], function (song) {
                allTracks.push(song);
                playlist.push(song);
                $('#list-group').append($(returnTrackHTML(song, playlist.length-1)));
            });
        }
    }
});
...
function traverseFileTree(item,path) {
    ...
    if(item.isFile){
        item.file(function(file){
            if(file.type.match(/audio\/(mp3|mpeg|m4a)/)){
...
function getTags(file,done){
    ...
    ID3.loadTags(file.name, function() {
        ...
        if(tags.picture && tags.picture.data && tags.picture.data.length) {
            ...
            result.picture = 'images/doge.jpeg';
            done(result);
        }
...
function returnTrackHTML(song, index){
    ...
    var html = '<li class="track list-group-item"';
    ...
    '<div>' +
    '<span class="overlay"><i class="glyphicon glyphicon-play"></i></span>' +
